	...
	factory: function(require) {
					...
					class WorkspacesForeachCommand extends cli_1.BaseCommand {
						...
						async execute() {
							...
							for (const workspace of candidates) {
								if (
									scriptName &&
									!workspace.manifest.scripts.has(scriptName)
								)
								...
								if (
									...
									!this.include.includes(
										core_3.structUtils.stringifyIdent(
											workspace.locator
										)
									)
								)
					...
					WorkspacesForeachCommand.schema = yup.object().shape({
						...
						parallel: yup.boolean().when(`jobs`, {
							...
							then: yup
								...
								.oneOf(
									[true],
									`--parallel must be set when using --jobs`
								),
      ...
      `,
					...
					function getPrefix(
						...
						const ident = core_3.structUtils.convertToIdent(
							workspace.locator
						);
