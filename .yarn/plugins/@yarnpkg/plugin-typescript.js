			...
			/******/ [
				...
				/***/ function(module, exports, __webpack_require__) {
					...
					const afterWorkspaceDependencyAddition = async (
						...
						const requiresInstallTypes = await typescriptUtils_1.hasDefinitelyTyped(
							descriptor,
							configuration
						);
						...
						const target =
							plugin_essentials_1.suggestUtils.Target.DEVELOPMENT;
						const modifier =
							plugin_essentials_1.suggestUtils.Modifier.EXACT;
						const strategies = [
							plugin_essentials_1.suggestUtils.Strategy.LATEST,
						];
						const request = core_2.structUtils.makeDescriptor(
							core_2.structUtils.makeIdent(`types`, typesName),
							`unknown`
						);
						const suggestions = await plugin_essentials_1.suggestUtils.getSuggestedDescriptors(
							request,
							{
								workspace,
								project,
					...
					const afterWorkspaceDependencyRemoval = async (
						...
						const target =
							plugin_essentials_1.suggestUtils.Target.DEVELOPMENT;
						const typesName = getTypesName(descriptor);
						const ident = core_2.structUtils.makeIdent(
							`types`,
							typesName
						);
						const current = workspace.manifest[target].get(
							ident.identHash
						);
					...
					const beforeWorkspacePacking = (workspace, rawManifest) => {
						if (
							...
							rawManifest.publishConfig.typings
						)
							rawManifest.typings =
								rawManifest.publishConfig.typings;
						...
						if (
							rawManifest.publishConfig &&
							rawManifest.publishConfig.types
						) {
				...
				/***/ function(module, exports, __webpack_require__) {
					...
					const algoliasearch_1 = __importDefault(
					...
					exports.hasDefinitelyTyped = async (
						descriptor,
						configuration
					) => {
						var _a;

						const stringifiedIdent = core_1.structUtils.stringifyIdent(
							descriptor
						);
						...
						try {
							const packageInfo = await index.getObject(
								stringifiedIdent,
								{
							...
							return (
								((_a = packageInfo.types) === null ||
								_a === void 0
									? void 0
									: _a.ts) === "definitely-typed"
							);
						} catch (_e) {
							return false;
						}
					...
					const createAlgoliaClient = configuration => {
						const requester = {
							async send(request) {
								try {
									const response = await core_1.httpUtils.request(
										request.url,
										request.data || null,
										{
											configuration,
						...
						return algoliasearch_1.default(
							ALGOLIA_APP_ID,
							ALGOLIA_API_KEY,
							{
								requester,
					...
					function algoliasearch(appId, apiKey, options) {
						const commonOptions = {
							...
							userAgent: transporter
								.createUserAgent(clientCommon.version)
								.add({
									segment: "Node.js",
						...
						return clientSearch.createSearchClient({
							...
							methods: {
								...
								multipleGetObjects:
									clientSearch.multipleGetObjects,
								...
								generateSecuredApiKey:
									clientSearch.generateSecuredApiKey,
								getSecuredApiKeyRemainingValidity:
									clientSearch.getSecuredApiKeyRemainingValidity,
								destroy: clientCommon.destroy,
								initIndex: base => indexName => {
									return clientSearch.initIndex(base)(
										indexName,
										{
											methods: {
												batch: clientSearch.batch,
												delete:
													clientSearch.deleteIndex,
												getObject:
													clientSearch.getObject,
												getObjects:
													clientSearch.getObjects,
												saveObject:
													clientSearch.saveObject,
												saveObjects:
													clientSearch.saveObjects,
												search: clientSearch.search,
												searchForFacetValues:
													clientSearch.searchForFacetValues,
												waitTask: clientSearch.waitTask,
												setSettings:
													clientSearch.setSettings,
												getSettings:
													clientSearch.getSettings,
												partialUpdateObject:
													clientSearch.partialUpdateObject,
												partialUpdateObjects:
													clientSearch.partialUpdateObjects,
												deleteObject:
													clientSearch.deleteObject,
												deleteObjects:
													clientSearch.deleteObjects,
												deleteBy: clientSearch.deleteBy,
												clearObjects:
													clientSearch.clearObjects,
												browseObjects:
													clientSearch.browseObjects,
												getObjectPosition:
													clientSearch.getObjectPosition,
												findObject:
													clientSearch.findObject,
												exists: clientSearch.exists,
												saveSynonym:
													clientSearch.saveSynonym,
												saveSynonyms:
													clientSearch.saveSynonyms,
												getSynonym:
													clientSearch.getSynonym,
												searchSynonyms:
													clientSearch.searchSynonyms,
												browseSynonyms:
													clientSearch.browseSynonyms,
												deleteSynonym:
													clientSearch.deleteSynonym,
												clearSynonyms:
													clientSearch.clearSynonyms,
												replaceAllObjects:
													clientSearch.replaceAllObjects,
												replaceAllSynonyms:
													clientSearch.replaceAllSynonyms,
												searchRules:
													clientSearch.searchRules,
												getRule: clientSearch.getRule,
												deleteRule:
													clientSearch.deleteRule,
												saveRule: clientSearch.saveRule,
												saveRules:
													clientSearch.saveRules,
												replaceAllRules:
													clientSearch.replaceAllRules,
												browseRules:
													clientSearch.browseRules,
												clearRules:
													clientSearch.clearRules,
											},
										}
									);
								},
									...
									return clientAnalytics.createAnalyticsClient(
										{
											...
											methods: {
												addABTest:
													clientAnalytics.addABTest,
												getABTest:
													clientAnalytics.getABTest,
												getABTests:
													clientAnalytics.getABTests,
												stopABTest:
													clientAnalytics.stopABTest,
												deleteABTest:
													clientAnalytics.deleteABTest,
											},
										}
									);
								},
				...
				/***/ function(module, exports, __webpack_require__) {
					...
					function createAuth(authMode, appId, apiKey) {
						...
						return {
							headers() {
								return authMode === AuthMode.WithinHeaders
									? credentials
									: {};
							},
							queryParameters() {
							...
							wait(requestOptions) {
								return createWaitablePromise(
									promise
										...
										.then(
											promiseResults => promiseResults[1]
										)
								);
							},
						});
				...
				/***/ function(module, exports, __webpack_require__) {
					...
					function createMappedRequestOptions(
						requestOptions,
						timeout
					) {
						...
						return Promise.all(
							statelessHosts.map(statelessHost => {
								return hostsCache.get(statelessHost, () => {
									return Promise.resolve(
										createStatefulHost(statelessHost)
									);
								});
							})
						).then(statefulHosts => {
							const hostsUp = statefulHosts.filter(host =>
								isStatefulHostUp(host)
							);
							...
							const statelessHostsAvailable =
								hostsAvailable.length > 0
									? hostsAvailable.map(host =>
											createStatelessHost(host)
									  )
									: statelessHosts;
							return {
								getTimeout(timeoutsCount, baseTimeout) {
									...
									const timeoutMultiplier =
										hostsTimeouted.length === 0 &&
										timeoutsCount === 0
											? 1
											: hostsTimeouted.length +
											  3 +
											  timeoutsCount;
					...
					function retryableRequest(
						...
						const retry = (
							...
							const decisions = {
								...
								onRetry(response) {
									const stackFrame = pushToStackTrace(
										response
									);
									/**
							...
							read(request, requestOptions) {
								...
								return transporter.responsesCache.get(
									...
									() => {
										 ...
										 */
										return transporter.requestsCache.get(
											key,
											() => {
												return (
													transporter.requestsCache
														...
														.then(
															response =>
																Promise.all([
																	transporter.requestsCache.delete(
																		key
																	),
																	response,
																]),
									...
									},
									{
										 ...
										 */
										miss: response =>
											transporter.responsesCache.set(
												key,
												response
											),
									}
					...
					function createUserAgent(version) {
						const userAgent = {
							...
							add(options) {
								...
								if (
									userAgent.value.indexOf(addedUserAgent) ===
									-1
								) {
									// eslint-disable-next-line functional/immutable-data
					...
					function stackFrameWithoutCredentials(stackFrame) {
						const modifiedHeaders = stackFrame.request.headers[
							"x-algolia-api-key"
						]
							? { "x-algolia-api-key": "*****" }
							: {};
				...
				/***/ function(module, exports, __webpack_require__) {
						...
						return (apiKey, requestOptions) => {
							...
							return clientCommon.createWaitablePromise(
								base.transporter.write(
									{
										method:
											requesterCommon.MethodEnum.Delete,
										path: clientCommon.encode(
											"1/keys/%s",
											apiKey
										),
					...
					const multipleBatch = base => {
						return (requests, requestOptions) => {
							const wait = (response, waitRequestOptions) => {
								return Promise.all(
									Object.keys(response.taskID).map(
										indexName => {
											return initIndex(base)(indexName, {
												methods: { waitTask },
											}).waitTask(
												response.taskID[indexName],
												waitRequestOptions
											);
										}
									)
					...
					const multipleQueries = base => {
						return (queries, requestOptions) => {
							const requests = queries.map(query => {
								return {
									...query,
									params: transporter.serializeQueryParameters(
										query.params || {}
									),
								};
							});
							...
							return createBrowsablePromise({
								...
								request: data =>
									base.transporter.read(
										{
											method:
												requesterCommon.MethodEnum.Post,
											path: clientCommon.encode(
												"1/indexes/%s/browse",
												base.indexName
											),
						...
						return (bodies, action, requestOptions) => {
							...
							const forEachBatch = (lastIndex = 0) => {
								...
								return batch(base)(
									...
									response.objectIDs = response.objectIDs.concat(
										res.objectIDs
							...
							return clientCommon.createWaitablePromise(
								...
								(chunkedBatchResponse, waitRequestOptions) => {
									return Promise.all(
										chunkedBatchResponse.taskIDs.map(
											taskID => {
												return waitTask(base)(
													taskID,
													waitRequestOptions
					...
					const clearRules = base => {
						return requestOptions => {
							const { forwardToReplicas, ...options } =
								requestOptions || {};
							const mappedRequestOptions = transporter.createMappedRequestOptions(
								options
							);
					...
					const getObjectPosition = () => {
						return (searchResponse, objectID) => {
							// eslint-disable-next-line functional/no-loop-statement
							for (const [position, hit] of Object.entries(
								searchResponse.hits
							)) {
						...
						return (objects, requestOptions) => {
							...
							const result = (safe
								...
								.then(() => {
									...
									return safe
										? saveObjectsWaitablePromise.wait(
												options
										  )
								...
								(_, waitRequestOptions) => {
									return Promise.all(
										responses.map(response =>
											response.wait(waitRequestOptions)
										)
									);
								}
							);
				...
				/***/ function(module, exports, __webpack_require__) {
					...
					/* eslint sonarjs/cognitive-complexity: 0 */ function createNodeHttpRequester() {
